...
int main(int argc, char *argv[]){
        ...
        for(int i = 0; i < n; i++){
            ...
            for(int j = 0; j < n; j++){
                if(j != i){ // exceto o x da diagonal
                    aux += (coef[i][j] * ant[j]); // o coeficiente de x vezes a solução anterior
                }else{
                    if(coef[i][j] == 0){ 
            ...
            // calcula...
            if(i > 0){
                if(fabs(sol[i] - ant[i]) > e)e = fabs(sol[i] - ant[i]); // o maior erro
                if(sol[i] > sol[i - 1])x = sol[i]; // o maior x
            }else e = fabs(sol[i] - ant[i]);
        }

        for(int i = 0; i < n; i++)ant[i] = sol[i]; // atualiza o vetor das soluções anteriores
    ...
    if(c >= max && rel > y){
        printf("A solução não foi atingida com %d iterações e o erro relativo foi de %f.\n", c, rel);
        return -1;
    }else if(isnan(rel)){
    ...
    printf("São as soluções com erro relativo %f:\n", rel);
